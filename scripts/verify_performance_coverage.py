...
- skip 契约:pipeline/audit/skip_performance.yaml 须为条目列表,每条 scene_id
  ...
  → 全局报错 exit 1,不只跳过该条
- 最小结构:performance 文件须含 lines / forbidden 两个顶层 key(文本级)
"""
import argparse
import re
import sys
...
TOPLEVEL_KEY_RE = {
    ...
    "forbidden": re.compile(r"^forbidden\s*:", re.MULTILINE),
}
...
def check_scene(work_dir, sid, skip_map):
    ...
    for p in perf_files:
        ...
        missing_keys = [k for k, rx in TOPLEVEL_KEY_RE.items() if not rx.search(text)]
        if missing_keys:
    ...
    if not expected:
        detail.append("role_briefs.md 缺失或无 character: 段")
    problems.append(

...
- **lint 命中不归本组处置**：ai_filler 病灶（含 banned_markdown 等 hard cliche）的修复指令由机器通道脚本直接生成，本组**不做逐 hit triage、不写豁免、不产 ledger**。本组只报 lint 看不见的语义维度（无功能修辞 / 格言金句 / 比喻堆砌），finding 照常写 severity 字段进 severity 数据流。

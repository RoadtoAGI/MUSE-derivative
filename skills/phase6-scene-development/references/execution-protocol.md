...
phase6_dispatcher(scenes_in_order):
    for scene in scenes_in_order:
        ...
        run_script("extract_scene_card.py", scene_id=scene.id, work_dir=WORKDIR)
            ...
            # ============ Step 4b: Phase 6 scene-level rehearsal（writer 前轻量排练）============
            # **前置依赖**：role_briefs.md（合集，全部在场角色各一段 YAML）已由 Step 2.1
            # role-brief-deriver 落盘（本 §1 dispatcher 的第一步——rehearsal prompt 第 3 条硬引用该文件）
            #
            # 为本场景每个 participant dispatch character-actor subagent 做 scene-level rehearsal
            # 产出 pipeline/staging/scene_{scene.id}/{slug}_rehearsal.md（四项固定契约见
            # character-rehearsal skill）
            #
            # 触发：rehearsal default-on，由 orchestrator 现场判 should_rehearse(scene.card)
            # 决定是否跳过——典型低价值场景（单角色独白 / 无对白过渡 / 非转折）可跳过，
            # 不规定数字门槛。rehearsal 文件存在 → writer preflight 自动并入 audit；
            # 不存在 → writer 兜底"仅凭 role_brief + skill package"，不阻断场景推进。
            #
            # 失败语义：单角色 rehearsal 失败 → 该角色无 rehearsal 文件，writer 按"仅凭
            # role_brief + skill package"兜底；不阻断场景推进。全部角色 rehearsal 失败也
            # 不阻断 — writer 仍能产 draft，只是人物语言素材弱。
            #
            # 静态 / 动态分层（§0.4 约定）：
            # - 元配置：${CLAUDE_PLUGIN_ROOT}/agents/character-actor.md
            # - 职责：${CLAUDE_PLUGIN_ROOT}/skills/character-rehearsal/references/situational-method.md
            # - orchestrator 动态传：scene_id / role_slug / 场景简短情境 / 输出路径
            #   文件路径硬约定（adapter / staging 输出）由 agent 静态读 situational-method
            #
            # **默认 = per-participant fan-out**：orchestrator 在 scene loop 内对每个
            # participant 直接 dispatch character-actor，**不**在 character-actor 内部嵌套
            # spawn。per-role 注意力隔离是声音独立性的前提；一个 actor 同时演多角色等于
            # 回到"场景语言整理器"，丢失隔离。
            if should_rehearse(scene.card):  # 见上方跳过条件；默认 True
                for participant in scene.card.participants:
                    role_slug = resolve_role_slug(participant)  # 从 build-meta.yaml / phase2_character.yaml 查
                    scene_situation = build_scene_situation(scene.card, participant)  # 1-2 句简述
                    output_path = f"pipeline/staging/scene_{scene.id}/{role_slug}_rehearsal.md"

                    rehearsal_result = Task(
                        subagent_type="character-actor",
                        prompt=(
                            # role package 走 file Read（非 skill registry 加载）：
                            # run 级 role package 不在项目级 skill registry，两个 runtime 统一走 file Read，
                            # 不是 skill 机制加载
                            f"你是角色 {participant.display_name}（role_slug: {role_slug}）。\n"
                            f"角色 runtime package 文件读取顺序（优先级从高到低）：\n"
                            f"  1. Read pipeline/story-character-skills/.claude/skills/{role_slug}/SKILL.md——长期人格权威源（skill-shaped role package，非 registry-level skill invocation）\n"
                            f"  2. Read pipeline/story-character-skills/.claude/skills/{role_slug}/state.md——本场景进入前的主观状态\n"
                            f"  3. Read pipeline/scene_{scene.id}/role_briefs.md——本场景 role_brief 合集，定位 character: {role_slug} 的 YAML 段读取你的目标 / 压力 / 误读 / stake / boldness_guardrails\n"
                            f"  fallback（仅在 1/2 文件缺失时）：Read pipeline/characters/{participant.display_name}.md（adapter，长期声线的兼容视图）\n"
                            f"\n"
                            f"当前场景 {scene.id} 情境：{scene_situation}\n"
                            f"\n"
                            f"按 character-rehearsal situational-method §4 产出四项固定内容"
                            f"（想说但不会直说 / 台词候选 2-4 条 / 禁用语气 / 动作或停顿），"
                            f"Write 到 {output_path}。产出必须承接本场景 role_brief 的目标/压力/误读，不得只泛化长期声线。\n"
                            f"\n"
                            f"完成回复 'done rehearsal for {role_slug} at scene {scene.id}'。"
                        ),
                    )
                    # rehearsal 失败不升级 ESCALATED——继续下一角色；
                    # 该角色无 rehearsal 文件，writer 按"仅凭 role_brief + skill package"兜底
...
| 层 | 执行 | 产出 |
|---|---|---|
| L1 脚本 lint（Phase A） | orchestrator 对每场景调三脚本 | `pipeline/review/lint/{scene_id}.{ai_filler,lexical_stats,dialogue}.yaml` |
...
def phase6_5_review(scenes_in_order):
    ...
    for scene in scenes_in_order:
        ...
        # ============ L3 verdict 路由 ============
        if verdict == "PASS":
            continue  # 已 publish 版即权威，直接进 Phase 7
...
- `pipeline/review/scene_{scene_id}.lint_resolution_ledger.yaml`
...
| 单条 character-actor dispatch 同时为本场所有角色产 rehearsal | 角色身份边界被破坏；多角色共用一个 actor 等于回到"场景语言整理器"，丢失 per-role 隔离 | 默认 per-participant fan-out：每 participant 一条 dispatch，只锚该角色的 `{slug}_rehearsal.md` |

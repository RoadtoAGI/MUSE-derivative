...
6. `pipeline/scene_{scene_id - 1}/draft_tail.md`（首场景 S01 无；`extract_draft_tail.py` 产；前 100 字尾摘供衔接）
7. `pipeline/phase0_conception.yaml` 的 `reference_materials.summary` + `reference_materials.key_details`（用户提供时有；只读这两个子字段，不读 `applicable_phases`）
8. `pipeline/phase1_world.yaml` 的 `domain_knowledge` 字段（Phase 1 有时读；全量读）
9. `pipeline/staging/scene_{scene_id}/{slug}_rehearsal.md`（Phase 6 scene-level rehearsal 产物，orchestrator 在 writer dispatch 前按需预跑——见 `phase6-scene-development/references/execution-protocol.md §1 rehearsal 伪代码`）
	   - 若本 scene 某 participant 有 rehearsal 文件 → 读该 participant 的文件获取本场景语言素材（四项固定：想说但不会直说 / 台词候选 2-4 条 / 禁用语气 / 动作或停顿）
	   - 若无（预跑失败 / 跳过 / 场景触发条件不满足）→ 按"仅凭 role_brief + skill package 写"兜底，不阻断
...
| 3️⃣ | `pipeline/staging/scene_{scene_id}/{slug}_rehearsal.md` | **本场景语言素材** — 想说但不会直说 / 台词候选 2-4 条 / 禁用语气 / 动作或停顿 | 场景级细颗粒度的台词 / 动作 / 潜台词提示；是 1️⃣+2️⃣ 的场景化投影 |

**组装方式**：

- 1️⃣ 决定"角色想做什么"、2️⃣ 决定"角色怎么说话"、3️⃣ 提供"这一刻具体怎么体现"
- 有 3️⃣ 时优先从候选台词 / 动作里挑选 + 改写（见下方禁语红线）；无 3️⃣ 时按 1️⃣+2️⃣ 自由发挥
- 三层出现冲突时以 1️⃣ 为准（本场景 primary_objective 优先于长期声音与局部台词）；若 3️⃣ 的候选与 2️⃣ 的声音框架明显不符，视为 rehearsal 产物质量问题，**忽略 3️⃣** 只用 1️⃣+2️⃣
...
- 不读 / 不写其他场景的文件
- **禁 rehearsal 原文照搬**：`{slug}_rehearsal.md` 的"台词候选"是**候选素材**不是成品台词；writer 必须**改写、截断、或只取意图**后写入正文——原文照搬会让多场景叙事出现重复句式、且破坏"文本来自角色此刻选择"的幻觉。候选之间也不应在同一场景一次性全部用上（给其他场景留空间）。
- **禁**把 rehearsal 文件的章节标题（"想说但不会直说" / "台词候选" / "禁用语气" / "动作或停顿"）写入正文（同 scene_tasks marker 禁令）
